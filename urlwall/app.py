...
def _ignore_bundle(src, names):
    ...
    ignored = []
    for pattern in ('__pycache__', '*.pyc', '.git', '.gitignore', 'tests',
...
def patchInfo(appPath):
    ...
    patch = plistlib.load(open(patchfn, 'rb'))
    info = plistlib.load(open(infofn, 'rb'))
    info.update(patch)
    plistlib.dump(info, open(infofn, 'wb'))
